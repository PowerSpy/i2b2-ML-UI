...
SCRIPT = '''import sys
sys.path.insert(0, "/usr/src/app")          # required: importdata/ is on sys.path, not repo root
from tests.ML.test_helper import create_patient_set
print(create_patient_set(
    project_id="demo", user_id="demo",
    query_name={name!r},
...
def create_cohort(name: str, concept_code: str) -> int:
    ...
    sql = f"SELECT patient_num FROM observation_fact WHERE concept_cd = '{concept_code}'"

    host_dir = Path(tempfile.mkdtemp())
    script = host_dir / "make_cohort.py"
    script.write_text(SCRIPT.format(name=name, sql=sql))
...
CREATE TEMP TABLE _delm AS
  SELECT DISTINCT query_master_id AS qmid FROM i2b2demodata.qt_query_instance
  WHERE query_instance_id IN (SELECT qiid FROM _del);
DELETE FROM i2b2demodata.qt_patient_set_collection WHERE result_instance_id IN (SELECT rid FROM _del);
DELETE FROM i2b2demodata.qt_patient_enc_collection WHERE result_instance_id IN (SELECT rid FROM _del);
DELETE FROM i2b2demodata.qt_xml_result WHERE result_instance_id IN (SELECT rid FROM _del);
DELETE FROM i2b2demodata.qt_query_result_instance WHERE result_instance_id IN (SELECT rid FROM _del);
DELETE FROM i2b2demodata.qt_query_instance qi
  WHERE qi.query_instance_id IN (SELECT qiid FROM _del)
    AND NOT EXISTS (SELECT 1 FROM i2b2demodata.qt_query_result_instance r
                    WHERE r.query_instance_id = qi.query_instance_id);
DELETE FROM i2b2demodata.qt_query_master qm
  WHERE qm.query_master_id IN (SELECT qmid FROM _delm)
    AND NOT EXISTS (SELECT 1 FROM i2b2demodata.qt_query_instance qi
...
def delete_cohorts(cohort_ids: list[int]) -> int:
    ...
    sql = DELETE_SQL.format(ids=ids)
    out = exec(settings.db_container,
               "psql -U i2b2 -d i2b2 -v ON_ERROR_STOP=1 --single-transaction "
               f"-c {shlex.quote(sql)}")
    if out.returncode != 0:
...
def live_size(cohort_id: int) -> int:
    return int(scalar(
        "SELECT count(*) FROM i2b2demodata.qt_patient_set_collection "
        f"WHERE result_instance_id = {int(cohort_id)} "
        "AND patient_num IN (SELECT DISTINCT patient_num FROM i2b2demodata.observation_fact);"
    ) or 0)
